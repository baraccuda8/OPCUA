...
namespace OpcUa
{
        ...
        : Server(server), Client(callback), Logger(logger)
    {
        try
        {
            ...
            Data = Server->Subscriptions()->CreateSubscription(request, [this, serverptr](PublishResult i) {

                if(serverptr.get() && serverptr.get())
                    this->PublishCallback(serverptr, i);
            });
        ...
        {
            LOG_WARN(Logger, __FUN("subscription          | Subscription::Subscription"));
        }
    }
    ...
    void Subscription::PublishCallback(Services::SharedPtr server, const PublishResult result)
    {
        ...
        LOG_DEBUG(Logger, __FUN("subscription          | Suscription::PublishCallback called with {} notifications"), result.NotificationMessage.NotificationData.size());
        ...
        for (const NotificationData& data : result.NotificationMessage.NotificationData)
        {
            ...
            {
                LOG_DEBUG(Logger, __FUN("subscription          | notification is of type DataChange"));
            ...
            {
                LOG_DEBUG(Logger, __FUN("subscription          | notification is of type Event"));
            ...
            {
                LOG_DEBUG(Logger, __FUN("subscription          | notification is of type StatusChange"));
            ...
            {
                LOG_WARN(Logger, __FUN("subscription          | unknown notficiation type received: {}"), data.Header.TypeId);
            }
        }
    ...
    void Subscription::CallDataChangeCallback(const NotificationData& data)
    {
        for (const MonitoredItems& item : data.DataChange.Notification)
        {
            ...
            {
                LOG_WARN(Logger, __FUN("subscription          | got PublishResult for an unknown monitoreditem id: {}"), item.ClientHandle);
            }
            ...
            else
            {
                ...
                LOG_DEBUG(Logger, __FUN("subscription          | calling DataChange user callback: {} and node: {}"), item.ClientHandle, mapit->second.TargetNode);
    ...
    void Subscription::CallEventCallback(const NotificationData& data)
    {
        for (EventFieldList ef : data.Events.Events)
        {
            ...
            if (mapit == AttributeValueMap.end())
            {
                LOG_WARN(Logger, __FUN("subscription          | got PublishResult for an unknown MonitoredItem id: {}"), ef.ClientHandle);
            }
            ...
            else
            {
                ...
                {
                    throw std::runtime_error(__FUN("subscription          | receive event format does not match requested filter"));
                }
                ...
                LOG_DEBUG(Logger, __FUN("subscription          | calling client event callback"));

                Client.Event(mapit->second.MonitoredItemId, ev);

                LOG_DEBUG(Logger, __FUN("subscription          | callback call finished"));
            }
        }
    }
    ...
    uint32_t Subscription::SubscribeDataChange(const Node& node, AttributeId attr)
    {
        ...
        if (results.size() != 1) { throw std::runtime_error(__FUN("subscription          | SubscribeDataChange should have returned exactly one result")); }
    ...
    std::vector<uint32_t> Subscription::SubscribeDataChange(const std::vector<ReadValueId>& attributes)
    {
        ...
        {
            throw (std::runtime_error(__FUN("subscription          | server did not send answer for all MonitoredItem requests")));
        }
        ...
        for (const auto& res : results)
        {
            ...
            LOG_DEBUG(Logger, __FUN("subscription          | storing monitoreditem with handle: {} and id: {}"), itemsParams.ItemsToCreate[i].RequestedParameters.ClientHandle, res.MonitoredItemId);
    ...
    void Subscription::UnSubscribe(std::vector<uint32_t> handles)
    {
        ...
        for (auto id : handles)
        {
            LOG_DEBUG(Logger, __FUN("subscription          | sending unsubscribe for MonitoredItem id: {}"), id);
    ...
    uint32_t Subscription::SubscribeEvents(const Node& node, const Node& eventtype)
    {
        ...
        LOG_DEBUG(Logger, __FUN("subscription          | subscribing events with filter for:"));

        for (Node& child : eventtype.GetProperties())
        {
            auto propertyName = child.GetBrowseName();
            LOG_DEBUG(Logger, __FUN("  property: {}"), propertyName);
    ...
    uint32_t Subscription::SubscribeEvents(const Node& node, const EventFilter& eventfilter)
    {
        ...
        if (results.size() != 1)
        {
            throw (std::runtime_error(__FUN("subscription          | CreateMonitoredItems should return one result")));
        }
