...
namespace OpcUa
{
    namespace Model
    {
        ...
        DataValue Variable::GetValue() const
        {
            ...
            if (result.size() != 1)
            {
                throw std::runtime_error(__FUN("Cannot read variable value. Server returned invalid number of values."));
            }
        ...
        void Variable::SetValue(const DataValue& value)
        {
            ...
            if (result.size() != 1)
            {
                throw std::runtime_error(__FUN("Failed to write data. Server returned wron nunber of status codes."));
            }
