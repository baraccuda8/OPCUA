...
namespace
{
    ...
    public:
        ...
        virtual ~AddonsManagerImpl()
        {
            ...
            {
                LOG_ERROR(Logger, __FUN("addons_manager | error: {}"), err.GetFullMessage());
            }

            catch (...)
            {
                LOG_ERROR(Logger, __FUN("addons_manager | unknown exception, terminating"));
                std::terminate();
            }
        }
    ...
    private:
        void StopAddons()
        {
            ...
            while (AddonData* addonData = GetNextAddonDataForStop())
            {
                ...
                {
                    LOG_ERROR(Logger, __FUN("addons_manager | failed to stop addon '{}': {}"), addonData->Id, exc.what());
                }
            }
        ...
        bool DoStart()
        {
            while (AddonData* addonData = GetNextAddonDataForStart())
            {
                ...
                {
                    LOG_ERROR(Logger, __FUN("addons_manager | failed to initialize addon '{}': {}"), addonData->Id, exc.what());
                    return false;
                }
