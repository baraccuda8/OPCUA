...
namespace OpcUa
{
    ...
    QualifiedName Node::GetBrowseName() const
    {
        ...
        {
            throw std::runtime_error(__FUN("Could not retrieve browse name."));
        }
    ...
    Node Node::GetChild(const std::vector<std::string>& path) const
    {
        ...
        uint16_t namespaceIdx = (uint16_t)Id.GetNamespaceIndex();
    ...
    {
        NodeId nodeid = NumericNodeId(0, (uint16_t)namespaceIdx);
