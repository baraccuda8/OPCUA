...
namespace OpcUa
{
    namespace Model
    {
        ...
        Variable Object::GetVariable(const RelativePath& relativePath) const
        {
            ...
            if (result.size() != 1)
            {
                throw std::runtime_error(__FUN("object_model| Server returned more than one browse paths on TranslateBrowsePathsToNodeIds request."));
            }
            ...
            {
                throw std::runtime_error(__FUN("object_model| Server returned too many target elements on TranslateBrowsePathsToNodeIds request."));
            }
        ...
        NodeId Object::InstantiateType(const NodeId& newNodeId, const NodeId& parentNode, const NodeId& typeId, NodeClass nodeClass, const QualifiedName& browseName, const std::string& displayName)
        {
            ...
            {
                throw std::runtime_error(__FUN("opcua_model| Server returned wrong number new nodes results."));
            }
