...
namespace OpcUa
{
    ...
    void KeepAliveThread::Run()
    {
        ...
        while (!StopRequest)
        {
            int64_t t_sleep = (int64_t)(Period * 0.7);
            ...
            params.RequestLifeTime = (int32_t)Period;
        ...
        LOG_DEBUG(Logger, "{:90}| stopped", FUNCTION_LINE_NAME);
    }
    ...
    void KeepAliveThread::Stop()
    {
        ...
        {
            LOG_ERROR(Logger, "{:90}| exception thrown at attempt to join: Uncovn Error", FUNCTION_LINE_NAME);
            throw;
        }
    }
    ...
    EndpointDescription UaClient::SelectEndpoint(const std::string& endpoint)
    {
        ...
        throw std::runtime_error(__FUN("No supported endpoints found on server"));
    }
    ...
    void UaClient::Connect(const EndpointDescription& endpoint)
    {
        ...
        ActivateSessionParameters sessionParameters;
        {
            ...
            {
                throw std::runtime_error(__FUN("Cannot find suitable user identify token for session"));
            }
        ...
        {
            DefaultTimeout = (int32_t)(createSessionResponse.Parameters.RevisedSessionTimeout);
        }
    ...
    void UaClient::Disconnect()
    {
        ...
        if (Server.get())
        {
            ...
            LOG_INFO(Logger, "{:90}| CloseSession response is {}", FUNCTION_LINE_NAME, response.Header.ServiceResult);
    ...
    {
        if (!Server) { throw std::runtime_error(__FUN("Not connected")); }
    ...
    uint32_t UaClient::GetNamespaceIndex(std::string uri)
    {
        ...
        throw std::runtime_error(__FUN("Error namespace uri does not exists in server"));
        //return -1;
    }
    ...
    void UaClient::DeleteNodes(std::vector<OpcUa::Node>& nodes, bool recursive)
    {
        ...
        LOG_DEBUG(Logger, __FUN("ua_client             | deleting nodes ..."));
    ...
    void UaClient::EncryptPassword(OpcUa::UserIdentifyToken&/*identity*/, const CreateSessionResponse& response)
    {
        ...
        LOG_DEBUG(Logger, __FUN("ua_client             | encrypting password RSA-OAEP"));
        ...
        LOG_DEBUG(Logger, __FUN("ua_client             | loading server certificate ... {}"), hex(response.Parameters.ServerCertificate.Data));
        ...
        if (ret != 0) {
            LOG_ERROR(Logger, __FUN("ua_client             | error loading server certificate {}"), error2string(ret));
            goto exit1;
        }
        {
            ...
            LOG_DEBUG(Logger, __FUN("ua_client             | seeding the random number generator..."));
            ret = mbedtls_ctr_drbg_seed(&ctr_drbg, mbedtls_entropy_func, &entropy, (const unsigned char*)pers, sizeof(pers));
            if (ret != 0) {
                LOG_ERROR(Logger, __FUN("ua_client             | error seeding the random number generator {}"), error2string(ret));
                goto exit2;
            }
            {
                ...
                LOG_DEBUG(Logger, __FUN("ua_client             | generating the RSA encrypted value..."));
                ...
                if (ret != 0) {
                    LOG_ERROR(Logger, __FUN("ua_client             | error RSA encryption {}"), error2string(ret));
                    goto exit2;
                }
                LOG_DEBUG(Logger, __FUN("ua_client             | encrypted password: {}"), hex(std::vector<unsigned char>(buff, buff + sizeof(buff))));
