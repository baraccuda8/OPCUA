...
namespace
{
    ...
    public:
        ...
        {
            LOG_TRACE(Logger, __FUN("binary_connection     | send: {}"), OpcUa::ToHexDump(message, size));
...
{
    const int sock = ConnectToRemoteHost(host, (unsigned short)port);
    return std::unique_ptr<RemoteConnection>(new BinaryConnection(sock, host, (unsigned short)port, logger));
}
