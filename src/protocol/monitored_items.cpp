    ...
    namespace Binary
    {
        ...
        void DataSerializer::Serialize<FilterOperand>(const FilterOperand& params)
        {
            *this << params.Header;
            ...
            if (params.Header.TypeId == ExpandedObjectId::ElementOperand)
            {
                uint32_t size = (uint32_t)RawSize(params.Element);
                *this << size;
                *this << params.Element;
            }
            ...
            {
                uint32_t size = (uint32_t)RawSize(params.Literal);
                *this << size;
                *this << params.Literal;
            }
            ...
            {
                uint32_t size = (uint32_t)RawSize(params.Attribute);
                *this << size;
                *this << params.Attribute;
            }
            ...
            {
                uint32_t size = (uint32_t)RawSize(params.SimpleAttribute);
                *this << size;
                *this << params.SimpleAttribute;
            }
        }
        ...
        std::size_t RawSize<MonitoringFilter>(const MonitoringFilter& data)
        {
            ...
            {
                throw std::runtime_error(__FUN("MonitoringFilter type not implemented"));
            }
        ...
        void DataDeserializer::Deserialize<MonitoringFilter>(MonitoringFilter& data)
        {
            *this >> data.Header;
            ...
            {
                throw std::runtime_error(__FUN("Filter data type not supported in deserialization"));
            }
        }
        ...
        void DataSerializer::Serialize<MonitoringFilter>(const MonitoringFilter& data)
        {
            *this << data.Header;
            ...
            {
                throw std::runtime_error(__FUN("Filter data type not supported in serialization"));
            }
        }
