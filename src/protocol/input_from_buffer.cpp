...
namespace OpcUa
{
    ...
    InputFromBuffer::InputFromBuffer(const char* buf, std::size_t bufSize)
        ...
        , Size(bufSize)
    {
        if (!Buffer) { throw std::invalid_argument(__FUN("InputFromBuffer: Input buffer cannot be empty.")); }

        if (!bufSize) { throw std::invalid_argument(__FUN("InputFromBuffer: Invalid size of buffer")); }
    }
