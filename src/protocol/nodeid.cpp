...
namespace OpcUa
{
    ...
    std::string NodeId::GetStringIdentifier() const
    {
        ...
        throw std::logic_error(__FUN("Node id is not in String format."));
    }
    ...
    uint32_t NodeId::GetIntegerIdentifier() const
    {
        ...
        default:
        {
            throw std::logic_error(__FUN("Cannot get integer identifier from NodeId - it is not in numeric format."));
        }
        }
    }
    ...
    void NodeId::SetNamespaceIndex(uint32_t ns)
    {
        ...
        {
        case EV_FOUR_BYTE:
            FourByteData.NamespaceIndex = (uint8_t)ns;
            return;

        case EV_NUMERIC:
            NumericData.NamespaceIndex = (uint16_t)ns;
            return;

        case EV_STRING:
            StringData.NamespaceIndex = (uint16_t)ns;
            return;

        case EV_GUId:
            GuidData.NamespaceIndex = (uint16_t)ns;
            return;

        case EV_BYTE_STRING:
            BinaryData.NamespaceIndex = (uint16_t)ns;
            return;
    ...
    void NodeId::CopyNodeId(const NodeId& node)
    {
        ...
        default:
        {
            throw std::logic_error(__FUN("Invalid Node Id encoding value."));
        }
        }
    ...
    NodeId::NodeId(MessageId messageId)
        ...
        , ServerIndex(0)
    {
        FourByteData.Identifier = (uint16_t)messageId;
    }
    ...
    NodeId::NodeId(ExpandedObjectId objectId)
        ...
        , ServerIndex(0)
    {
        FourByteData.Identifier = (uint16_t)static_cast<uint32_t>(objectId);
    }
    ...
    namespace Binary
    {
        ...
        std::size_t RawSize<NodeId>(const NodeId& id)
        {
            ...
            default:
                throw std::logic_error(__FUN("Unable serialize NodeId. Unknown encoding type."));
            };
        ...
        void DataDeserializer::Deserialize<OpcUa::NodeId>(OpcUa::NodeId& id)
        {
            *this >> id.Encoding;
            ...
            default:
            {
                throw std::logic_error(__FUN("Unable to deserialize NodeId. Unknown encoding type received."));
            }
            };
        ...
        void DataSerializer::Serialize<OpcUa::ExpandedNodeId>(const OpcUa::ExpandedNodeId& id)
        {
            *this << id.Encoding;
            ...
            default:
                throw std::logic_error(__FUN("Unable serialize ExpandedNodeId. Unknown encoding type."));
            };
