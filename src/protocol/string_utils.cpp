...
#include <limits>
    ...
    std::string OpcUa::ToString(const OpcUa::DateTime& t)
    {
        ...
        tm* curr_tm = localtime(&st);
        strftime(sdt, 50, "%F %T", curr_tm);
        return sdt;// std::ctime(&st);
    }
    ...
    OpcUa::NodeId OpcUa::ToNodeId(const std::string& data, uint32_t defaultNamespace)
    {
        ...
        if (nsString.empty())
        {
            ...
            {
                throw (std::runtime_error(__FUN("Namespace index coult not be parsed from string and not default index specified in string: " + data)));
            }
        }
        else
        {
        ...
        if (!integer.empty())
        {
            return OpcUa::NumericNodeId(GetInteger(integer), (uint16_t)ns);
        }

        const std::string str = GetNodeField(data, "s=");
        if (!str.empty())
        {
            return OpcUa::StringNodeId(str, (uint16_t)ns);
        }

        const std::string g = GetNodeField(data, "g=");
        if (!g.empty())
        {
            return OpcUa::GuidNodeId(ToGuid(g), (uint16_t)ns);
        }

        throw (std::runtime_error(__FUN("No identifier found in string: '" + data + "'")));
    }
    ...
    OpcUa::QualifiedName OpcUa::ToQualifiedName(const std::string& str, uint32_t default_ns)
    {
        ...
        {
            uint16_t ns = (uint16_t)std::stoi(str.substr(0, found));
        ...
        {
            throw (std::runtime_error(__FUN("Namespace index coult not be parsed from string and not default index specified in string: " + str)));
        }

        return QualifiedName((uint16_t)default_ns, str);
    }
