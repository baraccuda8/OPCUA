...
namespace
{
    ...
    void AsyncOpcTcpAddon::Initialize(Common::AddonsManager& addons, const Common::AddonParameters& addonParams)
    {
        ...
        LOG_DEBUG(Logger, __FUN("opc_tcp_async         | parameters:"));
        LOG_DEBUG(Logger, __FUN("opc_tcp_async         |   DebugMode: {}"), params.DebugMode);
        ...
        {
            LOG_ERROR(Logger, __FUN("opc_tcp_async         | Endpoints parameter not present in configuration"));
            return;
        }

        if (endpointDescriptions.size() > 1)
        {
            LOG_ERROR(Logger, __FUN("opc_tcp_async         | too many endpoints specified in configuration"));
            return;
        }
    ...
    void AsyncOpcTcpAddon::PublishApplicationsInformation(std::vector<OpcUa::ApplicationDescription> applications, std::vector<OpcUa::EndpointDescription> endpoints, const Common::AddonsManager& addons) const
    {
        ...
        if (!endpointsAddon)
        {
            LOG_ERROR(Logger, __FUN("opc_tcp_async         | cannot publish information about endpoints. Endpoints services addon not registered."));
            return;
        }
