...
namespace
{
    ...
    private:
        UserTokenType GetTokenType(const std::string& typeName) const
        {
            ...
            throw std::logic_error(__FUN("Unknown token type '" + typeName + "'"));
        }
        ...
        std::string GetTokenType(OpcUa::UserTokenType type) const
        {
            ...
            default:
                throw std::logic_error(__FUN("Unknown token type '" + std::to_string((unsigned)type) + "'"));
            }
        }
        ...
        ApplicationType GetApplicationType(const std::string& typeName) const
        {
            ...
            throw std::logic_error(__FUN("Invalid name of type application type: " + typeName));
        }
        ...
        std::string GetApplicationType(ApplicationType type) const
        {
            ...
            default:
                throw std::logic_error(__FUN("Unknown application type: " + std::to_string((unsigned)type)));
            }
        }
        ...
        MessageSecurityMode GetSecurityMode(const std::string& modeName) const
        {
            ...
            throw std::logic_error(__FUN("Unknown security mode name: " + modeName));
        }
        ...
        std::string GetSecurityMode(MessageSecurityMode mode) const
        {
            ...
            default:
                throw std::logic_error(__FUN("Unknown security mode: " + std::to_string((unsigned)mode)));
            }
        }
        ...
        UserTokenPolicy GetUserTokenPolicy(const std::vector<Common::Parameter>& params) const
        {
            ...
            for (const Common::Parameter& param : params)
            {
                if (param.Name == "id")
                {
                    tokenPolicy.PolicyId = param.Value;
                }//"Anonymous";
                ...
                {
                    tokenPolicy.SecurityPolicyUri = param.Value;
        ...
        EndpointDescription GetEndpointDescription(const Common::ParametersGroup& group) const
        {
            ...
            for (const Common::Parameter param : group.Parameters)
            {
                ...
                {
                    endpoint.SecurityLevel = (uint8_t)std::stoi(param.Value);
                }
                ...
                {
                    endpoint.TransportProfileUri = param.Value;
    ...
    private:
        ...
        {
            LOG_DEBUG(Logger, __FUN(msg), args...);
            /*
            if (!Debug)
              {
                return;
