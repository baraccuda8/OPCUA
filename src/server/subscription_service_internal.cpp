...
namespace
{
    OpcUa::ByteString GenerateEventId()
    {
        ...
        for (int i = 0; i < 8; i++)
        {
            int8_t val = (int8_t)(rand() % std::numeric_limits<int32_t>::max());
...
namespace OpcUa
{
    namespace Internal
    {
        ...
        {
            LOG_DEBUG(Logger, __FUN("subscription_service  | DeleteAllSubscriptions"));
        ...
        std::vector<StatusCode> SubscriptionServiceInternal::DeleteSubscriptions(const std::vector<uint32_t>& subscriptions)
        {
            ...
            for (const uint32_t& subid : subscriptions)
            {
                ...
                {
                    LOG_ERROR(Logger, __FUN("subscription_service  | got request to delete non existing SubscriptionId: {}"), subid);
                    result.push_back(StatusCode::BadSubscriptionIdInvalid);
                }

                else
                {
                    LOG_DEBUG(Logger, __FUN("subscription_service  | delete SubscriptionId: {}"), subid);
        ...
        ModifySubscriptionResponse SubscriptionServiceInternal::ModifySubscription(const ModifySubscriptionParameters& parameters)
        {
            ...
            if (itsub == SubscriptionsMap.end())
            {
                LOG_ERROR(Logger, __FUN("subscription_service  | got request to modify non existing SubscriptionId: {}"), subid);
                response.Header.ServiceResult = StatusCode::BadSubscriptionIdInvalid;
                return response;
            }

            LOG_DEBUG(Logger, __FUN("subscription_service  | modify SubscriptionId: {}"), subid);
        ...
        SubscriptionData SubscriptionServiceInternal::CreateSubscription(const CreateSubscriptionRequest& request, std::function<void(PublishResult)> callback)
        {
            ...
            LOG_DEBUG(Logger, __FUN("subscription_service  | CreateSubscription id: {}"), data.SubscriptionId);
        ...
        void SubscriptionServiceInternal::Publish(const PublishRequest& request)
        {
            ...
            if (PublishRequestQueues[session] < 100)
            {
                ...
                LOG_DEBUG(Logger, __FUN("subscription_service  | push PublishRequest for session: {}: available requests: {}"), session, PublishRequestQueues[session]);
            }
        ...
        bool SubscriptionServiceInternal::PopPublishRequest(NodeId node)
        {
            ...
            {
                LOG_ERROR(Logger, __FUN("subscription_service  | attempt to pop publish request for unknown session: {}"), node);

                if (Logger && Logger->should_log(spdlog::level::debug))
                {
                    for (auto i : PublishRequestQueues)
                    {
                        Logger->debug(__FUN("subscription_service  |   available session: {}"), i.first);
                    }
                }
                return false;
            }
            ...
            else
            {
                ...
                {
                    LOG_ERROR(Logger, __FUN("subscription_service  | unable to send response: no publish request for session: {}"), node);
                    return false;
                }

                else
                {
                    LOG_DEBUG(Logger, __FUN("subscription_service  | pop PublishRequest for session: {}: available requests: {}"), node, queue_it->second);
                    --queue_it->second;
                    return true;
                }
            }
