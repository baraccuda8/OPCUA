...
namespace OpcUa
{
    namespace Internal
    {
        ...
        std::vector<BrowseResult> AddressSpaceInMemory::Browse(const OpcUa::NodesQuery& query) const
        {
            ...
            LOG_TRACE(Logger, __FUN("address_space_internal| browse"));
            ...
            for (BrowseDescription browseDescription : query.NodesToBrowse)
            {
                ...
                {
                    Logger->trace(__FUN("address_space_internal| browsing"));
                    Logger->trace(__FUN("  NodeId: '{}'"), browseDescription.NodeToBrowse);
                    Logger->trace(__FUN("  ReferenceId: '{}'"), browseDescription.ReferenceTypeId);
                    Logger->trace(__FUN("  Direction: {}"), browseDescription.Direction);
                    Logger->trace(__FUN("  NodeClasses: {:#x}"), (unsigned)browseDescription.NodeClasses);
                    Logger->trace(__FUN("  ResultMask:  {:#x}"), (unsigned)browseDescription.ResultMask);
                }
                ...
                {
                    LOG_WARN(Logger, __FUN("address_space_internal| Node '{}' not found in the address space"), OpcUa::ToString(browseDescription.NodeToBrowse));
        ...
        DataValue AddressSpaceInMemory::GetValue(const NodeId& node, AttributeId attribute) const
        {
            ...
            if (nodeit == Nodes.end())
            {
                //      LOG_DEBUG(Logger, __FUN("address_space_internal| node not found: {}"), node);
            }
            ...
            else
            {
                ...
                {
                    //          LOG_DEBUG(Logger, __FUN("address_space_internal| node: {} has no attribute: {}"), node, ToString(attribute));
                }
                ...
                else
                {
                    ...
                    {
                        LOG_DEBUG(Logger, __FUN("address_space_internal| invoke registered callback"));

                        return attrit->second.GetValueCallback();
                    }

                    //          LOG_TRACE(Logger, __FUN("address_space_internal| no callback registered, returning stored value"));
        ...
        uint32_t AddressSpaceInMemory::AddDataChangeCallback(const NodeId& node, AttributeId attribute, std::function<Server::DataChangeCallback> callback)
        {
            ...
            LOG_DEBUG(Logger, __FUN("address_space_internal| set data changes callback for node {} and attribute {}"), node, (unsigned)attribute);

            NodesMap::iterator it = Nodes.find(node);

            if (it == Nodes.end())
            {
                LOG_ERROR(Logger, __FUN("address_space_internal| Node: '{}' not found"), node);
                throw std::runtime_error(__FUN("address_space_internal| NodeId not found"));
            }
            ...
            {
                LOG_ERROR(Logger, __FUN("address_space_internal| Attribute: {} of node: ‘{}‘ not found"), (unsigned)attribute, node);
                throw std::runtime_error(__FUN("Attribute not found"));
            }
        ...
        void AddressSpaceInMemory::DeleteDataChangeCallback(uint32_t serverhandle)
        {
            ...
            LOG_DEBUG(Logger, __FUN("address_space_internal| deleting callback with client id: {}"), serverhandle);

            ClientIdToAttributeMapType::iterator it = ClientIdToAttributeMap.find(serverhandle);

            if (it == ClientIdToAttributeMap.end())
            {
                LOG_WARN(Logger, __FUN("address_space_internal| request to delete a callback using unknown handle: {1}"), serverhandle);
                return;
            }
            ...
            if (nodeit != Nodes.end())
            {
                ...
                if (ait != nodeit->second.Attributes.end())
                {
                    ...
                    LOG_DEBUG(Logger, __FUN("address_space_internal| deleted {} callbacks"), nb);
            ...
            throw std::runtime_error(__FUN("address_space_internal| NodeId or attribute nor found"));
        }
        ...
        void AddressSpaceInMemory::SetMethod(const NodeId& node, std::function<std::vector<OpcUa::Variant>(NodeId context, std::vector<OpcUa::Variant> arguments)> callback)
        {
            ...
            {
                throw std::runtime_error(__FUN("address_space_internal| while setting node callback: node does not exist."));
            }
        }
        ...
        CallMethodResult AddressSpaceInMemory::CallMethod(CallMethodRequest request)
        {
            ...
            {
                LOG_ERROR(Logger, __FUN("address_space_internal| exception while calling method: {}: {}"), request.MethodId, ex.what());
        ...
        {
            //  LOG_TRACE(Logger, __FUN("address_space_internal| checking reference: '{}' to node: '{}' ({}) which must fit ref: '{}' with IncludeSubtypes: '{}'"), reference.ReferenceTypeId, reference.TargetNodeId, reference.BrowseName, desc.ReferenceTypeId, desc.IncludeSubtypes);

            if ((desc.Direction == BrowseDirection::Forward && !reference.IsForward) || (desc.Direction == BrowseDirection::Inverse && reference.IsForward))
            {
                //      LOG_TRACE(Logger, __FUN("address_space_internal| reference in wrong direction"));
                return false;
            }

            if (desc.ReferenceTypeId != ObjectId::Null && !IsSuitableReferenceType(reference, desc.ReferenceTypeId, desc.IncludeSubtypes))
            {
                //      LOG_TRACE(Logger, __FUN("address_space_internal| reference has wrong type"));
                return false;
            }

            if (desc.NodeClasses != NodeClass::Unspecified && (desc.NodeClasses & reference.TargetNodeClass) == NodeClass::Unspecified)
            {
                //      LOG_TRACE(Logger, __FUN("address_space_internal| reference has wrong class"));
                return false;
            }

            //  LOG_TRACE(Logger, __FUN("address_space_internal| reference suitable"));
            return true;
        }
        ...
        AddNodesResult AddressSpaceInMemory::AddNode(const AddNodesItem& item)
        {
            ...
            LOG_TRACE(Logger, __FUN("address_space_internal| adding new node id: '{}' name: '{}'"), item.RequestedNewNodeId, item.BrowseName.Name);
            ...
            {
                LOG_ERROR(Logger, __FUN("address_space_internal| NodeId: '{}' already exists"), resultId);
            ...
            if (item.ParentNodeId != NodeId())
            {
                ...
                {
                    LOG_ERROR(Logger, __FUN("address_space_internal| parent node '{}' does not exists"), item.ParentNodeId);
            ...
            LOG_TRACE(Logger, __FUN("address_space_internal| node added"));
        ...
        NodeId AddressSpaceInMemory::GetNewNodeId(const NodeId& id)
        {
            ...
            for (;;)
            {
                NodeId result = OpcUa::NumericNodeId(++MaxNodeIdNum, (uint16_t)idx);
                ...
                {
                    throw std::runtime_error(__FUN("address_space_internal| unable to assign new NodeId: range exceeded"));
                }
            }
        }
    }
