...
namespace
{
    ...
    void OpcTcpConnection::ReadNextData()
    {
        ...
        async_read(Socket, buffer(Buffer), transfer_exactly(GetHeaderSize()),
            [self](const boost::system::error_code& error, std::size_t bytes_transferred)
            {
                ...
                {
                    LOG_WARN(self->Logger, __FUN("opc_tcp_async         | failed to process message header: {}"), exc.what());
                }
            }
        );
    }
    ...
    void OpcTcpConnection::ProcessHeader(const boost::system::error_code& error, std::size_t bytes_transferred)
    {
        if (error)
        {
            LOG_ERROR(Logger, __FUN("opc_tcp_async         | error receiving message header: {}"), error.message());
            GoodBye();
            return;
        }

        LOG_DEBUG(Logger, __FUN("opc_tcp_async         | received message header with size: {}"), bytes_transferred);
        ...
        LOG_DEBUG(Logger, __FUN("opc_tcp_async         | received message: Type: {}, ChunkType: {}, Size: {}: DataSize: {}"), header.Type, header.Chunk, header.Size, messageSize);
    ...
    void OpcTcpConnection::ProcessMessage(OpcUa::Binary::MessageType type, const boost::system::error_code& error, std::size_t bytesTransferred)
    {
        if (error)
        {
            LOG_ERROR(Logger, __FUN("opc_tcp_async         | error receiving message body: {}"), error.message());
            GoodBye();
            return;
        }

        LOG_TRACE(Logger, __FUN("opc_tcp_async         | received message: {}"), ToHexDump(Buffer, bytesTransferred));
        ...
        {
            LOG_ERROR(Logger, __FUN("opc_tcp_async         | failed to process message: {}"), exc.what());
            GoodBye();
            return;
        }
        ...
        {
            std::cerr << __FUN("opc_tcp_async         | ERROR!!! Message from client has been processed partially.") << std::endl;
        }
    ...
    void OpcTcpConnection::GoodBye()
    {
        ...
        LOG_DEBUG(Logger, __FUN("opc_tcp_async         | good bye"));
    }
    ...
    void OpcTcpConnection::Send(const char* message, std::size_t size)
    {
        ...
        LOG_TRACE(Logger, __FUN("opc_tcp_async         | send message: {}"), ToHexDump(*data));
        ...
        async_write(Socket, buffer(&(*data)[0], data->size()), [self, data](const boost::system::error_code& err, size_t /*bytes*/)
            {
                if (err)
                {
                    LOG_ERROR(self->Logger, __FUN("opc_tcp_async         | failed to send data: {}"), err.message());
                    self->GoodBye();
                    return;
                }

                LOG_DEBUG(self->Logger, __FUN("opc_tcp_async         | response sent"));
            });
    }
        ...
        , acceptor(ioService)
    {
        ...
        {
            ep = tcp::endpoint(tcp::v4(), (boost::asio::ip::port_type)params.Port);
        }

        else if (params.Host == "localhost")
        {
            ep = tcp::endpoint(ip::address::from_string("127.0.0.1"), (boost::asio::ip::port_type)params.Port);
        }

        else
        {
            ep = tcp::endpoint(ip::address::from_string(params.Host), (boost::asio::ip::port_type)params.Port);
        }
    ...
    {
        LOG_DEBUG(Logger, __FUN("opc_tcp_async         | running server"));

        LOG_DEBUG(Logger, __FUN("opc_tcp_async         | waiting for client connection at: {}:{}"), acceptor.local_endpoint().address(), acceptor.local_endpoint().port());
        acceptor.listen();
    ...
    {
        LOG_DEBUG(Logger, __FUN("opc_tcp_async         | shutting down server"));
        acceptor.close();
    ...
    void OpcTcpServer::Accept()
    {
            ...
            acceptor.async_accept(socket, [this](boost::system::error_code errorCode)
                {
                    ...
                    if (!errorCode)
                    {
                        LOG_DEBUG(Logger, __FUN("opc_tcp_async         | accepted new client connection"));
                    ...
                    {
                        LOG_WARN(Logger, __FUN("opc_tcp_async         | error during client connection: {}"), errorCode.message());
                    }
        ...
        {
            LOG_WARN(Logger, __FUN("opc_tcp_async         | error accepting client connection: {}"), exc.what());
        }
    }
