...
namespace
{
...
#endif
            {
                LOG_ERROR(Logger, __FUN("unable to close server socket"));
            }
        }
    ...
    public:
        Client(std::shared_ptr<IOChannel> channel, std::shared_ptr<IncomingConnectionProcessor> processor, std::function<void()> onFinish, const Common::Logger::SharedPtr& logger)
            ...
            , Logger(logger)
        {
            LOG_DEBUG(Logger, __FUN("starting new client thread"));
        ...
        ~Client()
        {
            ClientThread->Join();
            ClientThread.reset();
            LOG_DEBUG(Logger, __FUN("client thread stopped"));
        }

    protected:
        virtual void OnSuccess()
        {
            LOG_DEBUG(Logger, __FUN("server thread exited successfully"));
        }

        virtual void OnError(const std::exception& exc)
        {
            LOG_ERROR(Logger, __FUN("server thread terminated: {}"), exc.what());
        }
    ...
    private:
        void Run()
        {
            try
            {
                LOG_DEBUG(Logger, __FUN("start to process client connection"));
                Processor->Process(Channel);
            }

            catch (const std::exception& exc)
            {
                LOG_ERROR(Logger, __FUN("unable to process client connection: {}"), exc.what());
            }
    ...
    public:
        ...
        virtual ~TcpServerConnection()
        {
            ...
            {
                LOG_ERROR(Logger, __FUN("unable to stop server: "), exc.what());
            }
        }
        ...
        virtual void Stop()
        {
            if (ServerThread)
            {
                LOG_DEBUG(Logger, __FUN("shutting down opc ua binary server"));
                Stopped = true;
#define SHUT_RDWR 2
    ...
    protected:
        ...
        virtual void OnError(const std::exception& exc)
        {
            LOG_ERROR(Logger, __FUN("server thread exited with error: {}"), exc.what());
            //throw 20;
        }
    ...
    private:
        void StartNewThread()
        {
            LOG_DEBUG(Logger, __FUN("starting new server thread"));
            Stopped = false;
        ...
        void Run()
        {
            ...
            if (Socket < 0)
            {
                throw std::logic_error(__FUN(std::string("unable to create server socket: ") + strerror(errno)));
            }

            SocketHolder holder(Socket, Logger);
            LOG_DEBUG(Logger, __FUN("listening on: {}"), Port);
            ...
            if (bind(Socket, (sockaddr*)&addr, sizeof(addr)) < 0)
            {
                ...
                throw std::logic_error(__FUN(std::string("unable bind socket: ") + strerror(errno)));
            }
            ...
            while (!Stopped)
            {
                int clientSocket = (int)accept(Socket, NULL, NULL);
                ...
                if (clientSocket < 0)
                {
                    throw std::logic_error(__FUN(std::string("unable to accept client connection: ") + strerror(errno)));
                }
    ...
    public:
        ...
        void Listen(const OpcUa::Server::TcpParameters& params, std::shared_ptr<OpcUa::Server::IncomingConnectionProcessor> processor) override
        {
            if (Servers.find(params.Port) != std::end(Servers))
            {
                ...
                throw std::logic_error(__FUN("server on this port already exists"));
            }
        ...
        void StopListen(const OpcUa::Server::TcpParameters& params) override
        {
            ...
            {
                LOG_ERROR(Logger, __FUN("stopping TcpServer on port: {} failed: {}"), params.Port, exc.what());
            }
        }
